...
| 项目 | IO 用法 |
|------|---------|
| 01-calc | `main` 读取 stdin，解析并求值 |
| 02-json-parser | `Test/Main.lean` 用 IO 运行测试断言 |
| 03-grep | `main` 读取命令行参数和文件内容 |
| 07-formal-verify | `main` 打印排序结果 |
| 08-type-checker | `main` 打印类型检查和求值结果 |

**03-grep — 读取文件并搜索（Main.lean）**

```lean
def main (args : List String) : IO UInt32 := do
  match parseArgs args with
  | .error msg => IO.println s!"错误：{msg}"; return 1
  | .ok config =>
    match config.filePaths with
    | [] => IO.println "请指定文件"; return 1
    | paths =>
      for path in paths do
        let content ← IO.FS.readFile ⟨path⟩
        let lines := content.splitOn "\n" |>.toArray
        let result := (searchLines lines).run config |> ExceptT.run
        IO.println s!"{path}: {result}"
      return 0
```
